package com.erpy.boardwang.board.AppZzang;
 ...
 */
public class AppZzangExtractorHumour {
    private static Logger logger = Logger.getLogger(AppZzangExtractorHumour.class.getName());
    ...
    public List<Board> extractList(Map<String, String> sourceMap) throws Exception {
        ...
        Elements elements = doc.select("div[class*=table-responsive]");
        for (Element element : elements) {
            Elements docSubElements = element.select("tr");
            for (Element docSubElement : docSubElements) {

                if (!docSubElement.outerHtml().contains("list-subject normal")) {
                    continue;
                }
                ...
                board.setCpNameDisplay("앱짱닷컴");

                /**
                 * link
                 */
                Elements docLinkElements = docSubElement.select("td[class*=list-subject] a");
...
//                }
    ...
    public Board extractContent(String body) throws Exception {
         ...
         * title
         */
        title = subExtractor.subExtract(body,"div[class*=view-wrap]", "h1", "text");
        if (title.length()>100) {
    ...
    void testExtract(String bodyFilePath) throws Exception {
        ...
        appZzangExtractorHumour.extractList(sourceMap);
    }
    ...
    void testExtractBody(String bodyUrl) throws Exception {
        ...
        Board board = appZzangExtractorHumour.extractContent(crawlContent.execute(bodyUrl, "utf-8"));
    ...
    public static void main(String args[]) throws Exception {
        AppZzangExtractorHumour appZzangExtractorHumour = new AppZzangExtractorHumour();
        appZzangExtractorHumour.testExtract("/Users/oj.bae/Work/BoardWang/crawl_data/AppZzangHumour_31996108.html");
